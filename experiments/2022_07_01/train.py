...
# TODO define random offset from the CT
from diffpose.ljubljana import get_random_offset

Transforms = Compose([Lambda(lambda x: (x - x.min()) / (x.max() - x.min() + 1e-6))])

def load(data_dir, orientation, subsample, device):
    # Load the subject
    data_dir = Path(data_dir)
    path = data_dir / "2022.07.01_FEM-NTI"
    vol_name = "CECT_no-contrast-1"
    vol_file = path / f"{vol_name}.nii.gz"
    subject = read(vol_file, 
               labelmap=None,
               orientation=orientation) 
    # volume[volume < 500] = 0.0
    # TODO automatic inital pose
    r = torch.tensor([[0., np.pi/5, 0.]], device=device)
    t = torch.tensor([[10., 700., 30.]], device=device)
    
    if orientation == 'PA':
        t = t * torch.tensor([-1., -1., 1.], device=device)

    isocenter_pose = convert(r, t, parameterization="euler_angles", convention="ZXY")
...
def main(
    ...
    drr, isocenter_pose, transforms = load(data_dir, orientation, subsample, device)
